...
from apps.mentions.utils.semantria.jsonserializer import *

#It's a THIRD_PARTY class WE ONLY USE IT!

class Session(object):
    host = 'https://api.semantria.com'
    wrapperName = 'Python/' + WRAPPER_VERSION

    def __init__(self, consumerKey, consumerSecret, serializer=None,
                 applicationName=None, use_compression=False):
        self.consumerKey = consumerKey
        self.consumerSecret = consumerSecret
        self.use_compression = use_compression

        if applicationName:
            self.applicationName = '{0}/{1}'.format(applicationName,
                                                    self.wrapperName)
        else:
    ...
    def getStatistics(self, configId=None, interval=None):
        """ Verify subscription responds with overall documents and system API
            calls balance

        :param configId: Identifier of the specific configuration for usage
        statistics retrieving
        :type configId: str
        ...
        if configId and interval and interval.lower() in ["hour", "day",
                                                          "week", "month",
                                                          "year"]:
    ...
    def getBlacklist(self, config_id=None):
        if config_id:
            url = '{0}/blacklist.{1}?config_id={2}'.format(self.host,
                                                           self.format,
                                                           config_id)
        else:
    ...
    def getCategories(self, config_id=None):
        if config_id:
            url = '{0}/categories.{1}?config_id={2}'.format(self.host,
                                                            self.format,
                                                            config_id)
        else:
    ...
    def getQueries(self, config_id=None):
        if config_id:
            url = '{0}/queries.{1}?config_id={2}'.format(self.host,
                                                         self.format,
                                                         config_id)
        else:
    ...
    def getPhrases(self, config_id=None):
        if config_id:
            url = '{0}/phrases.{1}?config_id={2}'.format(self.host,
                                                         self.format,
                                                         config_id)
        else:
    ...
    def getEntities(self, config_id=None):
        if config_id:
            url = '{0}/entities.{1}?config_id={2}'.format(self.host,
                                                          self.format,
                                                          config_id)
        else:
    ...
    def queueDocument(self, task, config_id=None):
        if config_id:
            url = '{0}/document.{1}?config_id={2}'.format(self.host,
                                                          self.format,
                                                          config_id)
        else:
    ...
    def queueBatch(self, batch, config_id=None):
        if config_id:
            url = '{0}/document/batch.{1}?config_id={2}'.format(self.host,
                                                                self.format,
                                                                config_id)
        else:
    ...
    def getDocument(self, doc_id, config_id=None):
        ...
        if config_id:
            url = '{0}/document/{1}.{2}?config_id={3}'.format(self.host,
                                                              doc_id,
                                                              self.format,
                                                              config_id)
        else:
    ...
    def getProcessedDocuments(self, config_id=None):
        if config_id:
            url = '{0}/document/processed.{1}?' \
                  'config_id={2}'.format(self.host, self.format, config_id)
        else:
    ...
    def getProcessedDocumentsByJobId(self, job_id):
        url = '{0}/document/processed.{1}?job_id={2}'.format(self.host,
                                                             self.format,
                                                             job_id)

        result = self._runRequest("GET", url,
                                  "get_processed_documents_by_job_id")
        if result is None:
            result = []
        return result
    ...
    def queueCollection(self, task, config_id=None):
        if config_id:
            url = '{0}/collection.{1}?config_id={2}'.format(self.host,
                                                            self.format,
                                                            config_id)
        else:
        ...
        data = self.serializer.serialize(task, wrapper)
        result = self._runRequest("POST", url, "get_processed_collections",
                                  data)
        if result is not None and not isinstance(result, int):
    ...
    def getCollection(self, coll_id, config_id=None):
        ...
        if config_id:
            url = '{0}/collection/{1}.{2}?config_id={3}'.format(self.host,
                                                                coll_id,
                                                                self.format,
                                                                config_id)
        else:
    ...
    def getProcessedCollections(self, config_id=None):
        if config_id:
            url = '{0}/collection/processed.{1}?config_id={2}'.format(
                self.host, self.format, config_id)
        else:
    ...
    def getProcessedCollectionsByJobId(self, job_id):
        url = '{0}/collection/processed.{1}?job_id={2}'.format(self.host,
                                                               self.format,
                                                               job_id)

        result = self._runRequest("GET", url,
                                  "get_processed_collections_by_job_id")
        if result is None:
            result = []
        return result
    ...
    def _getTypeWrapper(self, type_):
        ...
        if type_ == "update_configurations":
            return {"root": "configurations", "added": "configuration",
                    "removed": "configuration"}
        elif type_ == "update_blacklist":
            return {"root": "blacklist", "added": "item", "removed": "item"}
        elif type_ == "update_categories":
            return {"root": "categories", "added": "category",
                    "removed": "category", "samples": "sample"}
    ...
    def _runRequest(self, method, url, type_=None, postData=None):
        ...
        if method == "DELETE":
            ...
            if status == 200:
                ...
                message = self.serializer.deserialize(response["data"],
                                                      handler)
                return message
            elif status == 202:
    ...
    def _resolveError(self, status, message=None):
        if status == 400 or status == 401 or status == 402 or \
                        status == 403 or status == 406 or status == 500:
