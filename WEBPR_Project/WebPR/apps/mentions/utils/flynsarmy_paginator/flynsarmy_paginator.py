...
class FlynsarmyPaginator(Paginator):
    ...
    def __init__(self, object_list, per_page, orphans=0,
        ...
        # TODO: You should not supply class here. super() is all you need.
        super(FlynsarmyPaginator, self).__init__(object_list, per_page,
                                                 orphans,
    ...
    def page(self, number):
        ...
        if top + self.orphans >= self.count:
            top = self.count
        return FlynsarmyPage(self.object_list[bottom:top], number, self,
                             self.adjacent_pages)
...
class FlynsarmyPage(Page):
    ...
    def _get_page_range_data(self):
        ...
        page_range = [n for n in range(startPage, endPage) if
                      n > 0 and n <= self.paginator.count]

        return {
            'page_range': page_range,
            'show_first': 1 not in page_range and page_range,
            'show_last': (self.paginator.num_pages not in page_range and
                          page_range),
        }
